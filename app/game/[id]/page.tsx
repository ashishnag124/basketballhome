...
export default async function GamePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const game = await fetchGameBoxScore(id);
  if (!game) notFound();
  ...
  return (
    <div className="space-y-6">
      {/* Back link */}
      <Link href="/schedule" className="inline-flex items-center gap-1 text-sm text-[#003087] font-medium">
        ← Back to Schedule
      </Link>

      {/* Game header */}
      <div className="bg-[#003087] rounded-2xl p-5 text-white">
        <p className="text-[#B5A36A] text-xs font-bold uppercase tracking-widest mb-3 text-center">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-center gap-2 flex-1">
            <Image
              ...
              unoptimized
            />
            <span className="font-bold text-sm text-center">Duke Blue Devils</span>
            <span className="text-4xl font-bold font-['Oswald',sans-serif] text-[#B5A36A]">
              {game.dukeScore}
            </span>
          </div>
          ...
          <div className="text-center shrink-0">
            ...
            {isCompleted && (
              <div className={`mt-1 text-xs font-bold px-2 py-0.5 rounded ${dukeWon ? "bg-green-500/20 text-green-300" : "bg-red-500/20 text-red-300"}`}>
                {dukeWon ? "W" : "L"}
              </div>
            )}
          </div>
      ...
      {game.teams.map((team) => (
        <div key={team.teamId}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold text-[#001A57]">{team.teamName}</h2>
            {team.isDuke && <span className="text-xs bg-[#003087] text-white px-2 py-0.5 rounded font-bold">DUKE</span>}
          </div>
                  ...
                  {team.players.filter(p => p.starter && !p.didNotPlay).map((player) => (
                    <tr key={player.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      {player.stats.map((stat, i) => (
                        <td key={i} className={`text-center px-2 py-2 tabular-nums ${team.columns[i] === 'PTS' ? 'font-bold text-[#003087]' : 'text-gray-700'}`}>
                          {stat}
                        </td>
                      ))}
                    </tr>
