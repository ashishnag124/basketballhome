...
import { findDukeGame, fetchSchedule, fetchTeamStats, fetchRoster } from "@/lib/espn";
...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-6">
      {/* Hero banner */}
      <div className="relative rounded-2xl overflow-hidden bg-[#003087] min-h-[180px] flex items-center px-6 py-6">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -right-10 -top-10 w-64 h-64 bg-[#001A57] rounded-full opacity-40" />
          <div className="absolute -right-4 bottom-0 w-40 h-40 bg-[#B5A36A] rounded-full opacity-10" />
        </div>
        ...
        <div className="relative flex-1">
          <p className="text-[#B5A36A] text-xs font-bold uppercase tracking-widest mb-1">
            Duke Blue Devils
          </p>
          ...
          {stats && (
            <div className="mt-2 inline-flex items-center gap-2 bg-white/10 rounded-lg px-3 py-1">
              <span className="text-[#B5A36A] font-bold font-['Oswald',sans-serif] text-lg">
                {stats.record}
              </span>
            ...
            {featuredPlayers.slice(0, 3).map((player, i) => (
              ...
              >
                <Image
                  ...
                  unoptimized
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#001A57]/60 to-transparent" />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className="bg-white rounded-xl p-3 text-center shadow-sm">
                <div className="text-xl font-bold text-[#003087] font-['Oswald',sans-serif]">{s.value}</div>
      ...
      {featuredPlayers.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/roster" className="text-xs text-[#003087] font-medium">Full roster →</Link>
          </div>
            ...
            {featuredPlayers.map((player) => (
              <Link key={player.id} href="/roster" className="shrink-0">
                <div className="w-24 bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  <div className="relative h-28 bg-[#003087]">
                    <Image
                      src={player.photo!}
                      alt={player.name}
                      fill
                      ...
                      unoptimized
                    />
                    <div className="absolute top-1.5 left-1.5 bg-[#B5A36A] text-white text-[10px] font-bold rounded px-1">
                      #{player.jersey}
                    </div>
                  </div>
                  <div className="p-2 text-center">
                    <div className="text-[11px] font-bold text-[#001A57] truncate leading-tight">
      ...
      {recentResults.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/schedule" className="text-xs text-[#003087] font-medium">Full schedule →</Link>
          </div>
