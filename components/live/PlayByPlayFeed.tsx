...
function PlayItem({ play, teamId }: { play: EspnPlay; teamId: string }) {
  const isOurTeam = play.team?.id === teamId;

  return (
    <div
      className={`flex items-start gap-3 px-3 py-2.5 rounded-lg text-sm ${
        isOurTeam ? "bg-blue-50 border-l-2 border-[var(--color-primary)]" : "bg-gray-50"
      }`}
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`${isOurTeam ? "text-[var(--color-secondary)] font-medium" : "text-gray-600"} leading-snug`}>
          {play.text}
        </p>
...
export default function PlayByPlayFeed({ plays, teamId }: { plays: EspnPlay[]; teamId: string }) {
  if (!plays.length) {
    return (
  ...
  return (
    <div>
      ...
      <div className="space-y-1.5 max-h-72 overflow-y-auto">
        {plays.map((play) => (
          <PlayItem key={play.id} play={play} teamId={teamId} />
        ))}
      </div>
    </div>
  );
